...
class ValidationData:
	def __init__(self,h,device):
		...
		subset=int(os.environ.get('VAL_SUBSET_TOKENS','0'))
		if subset>0:
			usable=min(subset,self.val_tokens.numel()-1)//h.eval_seq_len*h.eval_seq_len
			self.val_tokens=self.val_tokens[:usable+1].contiguous()
...
class Block(nn.Module):
	...
	def forward(self,x,x0):
		...
		if self.parallel:
			mlp_out=self.mlp(self.mlp_norm(x_in)*self.ln_scale_factor);x_out=x_in+self.attn_scale.to(dtype=x_in.dtype)[None,None,:]*attn_out+self.mlp_scale.to(dtype=x_in.dtype)[None,None,:]*mlp_out
		else:
			x_out=x_in+self.attn_scale.to(dtype=x_in.dtype)[None,None,:]*attn_out;x_out=x_out+self.mlp_scale.to(dtype=x_out.dtype)[None,None,:]*self.mlp(self.mlp_norm(x_out)*self.ln_scale_factor)
		adapter_A=getattr(self,'ttt_layer_adapter_A',None);adapter_B=getattr(self,'ttt_layer_adapter_B',None)
		if adapter_A is not None and adapter_B is not None:
			feat=torch.matmul(x_out,adapter_A.to(dtype=x_out.dtype));x_out=x_out+float(getattr(self,'ttt_layer_adapter_scale',1.))*torch.matmul(feat,adapter_B.to(dtype=x_out.dtype))
		return x_out
class GPT(nn.Module):
	def __init__(self,h):
		super().__init__()
...
def _setup_ttt_adapter(h,device,model):
	rank=int(os.environ.get('TTT_ADAPTER_RANK','1024'))
	if rank<=0:return []
	gen=torch.Generator(device=device);gen.manual_seed(int(os.environ.get('TTT_ADAPTER_SEED',h.seed)))
	embed_dim=model.tok_emb.embedding_dim;A=torch.randn((embed_dim,rank),device=device,dtype=torch.float32,generator=gen)/math.sqrt(embed_dim)
	model.register_buffer('ttt_adapter_A',A.to(torch.bfloat16),persistent=False);model.ttt_adapter_B=nn.Parameter(torch.zeros(rank,embed_dim,device=device,dtype=torch.float32));model.ttt_adapter_scale=float(os.environ.get('TTT_ADAPTER_SCALE','1.0'))
	return [model.ttt_adapter_B]
def _parse_layer_list(spec,n):
	if not spec:return []
	...
	for part in spec.split(','):
		...
		if '-' in part:
			a,b=[int(x) for x in part.split('-',1)];out.extend(range(a,b+1))
		else:out.append(int(part))
	return sorted({i for i in out if 0<=i<n})
def _setup_layer_adapters(h,device,model):
	rank=int(os.environ.get('TTT_LAYER_ADAPTER_RANK','0'))
	layers=_parse_layer_list(os.environ.get('TTT_LAYER_ADAPTER_LAYERS',''),len(model.blocks))
	if rank<=0 or not layers:return []
	gen=torch.Generator(device=device);gen.manual_seed(int(os.environ.get('TTT_LAYER_ADAPTER_SEED',h.seed+271828)))
	params=[];scale=float(os.environ.get('TTT_LAYER_ADAPTER_SCALE','1.0'))
	for i in layers:
		block=model.blocks[i];A=torch.randn((h.model_dim,rank),device=device,dtype=torch.float32,generator=gen)/math.sqrt(h.model_dim)
		block.register_buffer('ttt_layer_adapter_A',A.to(torch.bfloat16),persistent=False);block.ttt_layer_adapter_B=nn.Parameter(torch.zeros(rank,h.model_dim,device=device,dtype=torch.float32));block.ttt_layer_adapter_scale=scale;params.append(block.ttt_layer_adapter_B)
	log(f"ttt_layer_adapters layers={layers} rank={rank} scale={scale}")
	return params
def _setup_causal_memory(h,device,model):
	if not bool(int(os.environ.get('TTT_CAUSAL_MEMORY','0'))):return
	model.register_buffer('ttt_memory_vec',torch.zeros(h.embedding_dim,device=device,dtype=torch.bfloat16),persistent=False);model.ttt_memory_scale=float(os.environ.get('TTT_MEMORY_SCALE','0.02'));model.ttt_memory_decay=float(os.environ.get('TTT_MEMORY_DECAY','0.9'));log(f"ttt_causal_memory scale={model.ttt_memory_scale} decay={model.ttt_memory_decay}")
def _update_causal_memory(model,tokens):
	mem=getattr(model,'ttt_memory_vec',None)
	if mem is None or tokens.numel()==0:return
	with torch.no_grad():
		emb=model.tok_emb(tokens).float().mean(dim=0);emb=emb/(emb.norm()+1e-6);decay=float(getattr(model,'ttt_memory_decay',0.9));mem.copy_((decay*mem.float()+(1-decay)*emb).to(mem.dtype))
...
def main():
	...
	if h.is_main_process:
		...
		mode=os.environ.get('EXPERIMENT_MODE','train_and_eval')
		if mode=='train_only':
			random.seed(h.seed);np.random.seed(h.seed);torch.manual_seed(h.seed);torch.cuda.manual_seed_all(h.seed);val_data=ValidationData(h,device);log(f'train_shards: {len(list(Path(h.datasets_dir).resolve().glob("fineweb_train_*.bin")))}');log(f'val_tokens: {val_data.val_tokens.numel()-1}');base_model,_=train_model(h,device,val_data);serialize(h,base_model,Path(__file__).read_text(encoding='utf-8'))
		elif mode=='eval_only':
			random.seed(h.seed);np.random.seed(h.seed);torch.manual_seed(h.seed);torch.cuda.manual_seed_all(h.seed);val_data=ValidationData(h,device);log(f'val_tokens: {val_data.val_tokens.numel()-1}');eval_model=deserialize(h,device)
			if h.num_loops>0:eval_model.looping_active=True
			label=os.environ.get('EVAL_LABEL','eval')
			if h.ttt_enabled and h.sliding_window_enabled:timed_eval(label,eval_val_ttt,h,device,val_data,eval_model)
			elif h.sliding_window_enabled:timed_eval(label,eval_val_sliding,h,device,val_data,eval_model)
			else:compiled_model=torch.compile(eval_model,dynamic=False,fullgraph=True);timed_eval(label,eval_val,h,device,val_data,compiled_model)
		else:train_and_eval(h,device)
