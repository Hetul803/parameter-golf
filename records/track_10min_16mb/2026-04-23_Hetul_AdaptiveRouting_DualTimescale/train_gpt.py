			...
			for(i,ws)in enumerate(batch_ws):
				...
				if lexical_cache is None:scored_nll=F.cross_entropy(logits[i,s:wlen].float(),tgt,reduction='none')
					...
					for pos in range(s,wlen):
						...
						biased_row=_lexical_cache_bias_logits(raw_row,x_batch[i,pos],lexical_cache,h,entropy=entropy);token_loss=F.cross_entropy(biased_row.unsqueeze(0),y_batch[i,pos].unsqueeze(0),reduction='none');scored_losses.append(token_loss);_lexical_cache_update(lexical_cache,x_batch[i,pos],y_batch[i,pos],h)
