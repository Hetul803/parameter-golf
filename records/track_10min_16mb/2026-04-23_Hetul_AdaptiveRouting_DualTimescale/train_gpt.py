	...
	with torch.inference_mode():
		for bi in range(0,len(my_windows),batch_seqs):
			...
			for(i,ws)in enumerate(batch_ws):
				...
				if lexical_cache is None and suffix_cache is None and rdc_state is None and ctr_state is None:scored_nll=F.cross_entropy(logits[i,s:wlen].float(),tgt,reduction='none')
				else:
					scored_losses=[]
					for pos in range(s,wlen):
						raw_row=logits[i,pos].float();entropy=None
