	...
	with torch.inference_mode():
		for bi in range(0,len(my_windows),batch_seqs):
			...
			for(i,ws)in enumerate(batch_ws):
				...
				if lexical_cache is None and suffix_cache is None:scored_nll=F.cross_entropy(logits[i,s:wlen].float(),tgt,reduction='none')
				else:
					scored_losses=[]
					for pos in range(s,wlen):
						...
						context_tokens=x_batch[i,:pos+1].tolist();biased_row=_lexical_cache_bias_logits(raw_row,x_batch[i,pos],lexical_cache,h,entropy=entropy);biased_row=_suffix_cache_bias_logits(biased_row,context_tokens,suffix_cache,h,entropy=entropy);token_loss=F.cross_entropy(biased_row.unsqueeze(0),y_batch[i,pos].unsqueeze(0),reduction='none');scored_losses.append(token_loss);_lexical_cache_update(lexical_cache,x_batch[i,pos],y_batch[i,pos],h);_suffix_cache_update(suffix_cache,context_tokens,y_batch[i,pos],h)
				...
				for(i,ws)in enumerate(batch_ws):
					...
					if lexical_cache is None and suffix_cache is None:scored_nll=F.cross_entropy(logits[i,s:wlen].float(),tgt,reduction='none')
						...
						for pos in range(s,wlen):
							...
							if h.entropy_weighted_lexical_cache:raw_probs=F.softmax(raw_row,dim=-1);entropy=-(raw_probs*torch.log(raw_probs.clamp_min(1e-09))).sum()
							if entropy is None and h.suffix_cache_entropy_gate:raw_probs=F.softmax(raw_row,dim=-1);entropy=-(raw_probs*torch.log(raw_probs.clamp_min(1e-09))).sum()
							context_tokens=x_batch[i,:pos+1].tolist();raw_token_loss=F.cross_entropy(raw_row.unsqueeze(0),y_batch[i,pos].unsqueeze(0),reduction='none');biased_row=_lexical_cache_bias_logits(raw_row,x_batch[i,pos],lexical_cache,h,entropy=entropy);biased_row=_suffix_cache_bias_logits(biased_row,context_tokens,suffix_cache,h,entropy=entropy);cached_token_loss=F.cross_entropy(biased_row.unsqueeze(0),y_batch[i,pos].unsqueeze(0),reduction='none');token_loss=cached_token_loss if h.use_suffix_cache else raw_token_loss;scored_losses.append(token_loss);chunk_credit_sum+=(raw_token_loss-cached_token_loss).to(torch.float64).sum();chunk_credit_count+=1.;_lexical_cache_update(lexical_cache,x_batch[i,pos],y_batch[i,pos],h);_suffix_cache_update(suffix_cache,context_tokens,y_batch[i,pos],h)
						scored_nll=torch.cat(scored_losses,dim=0)if scored_losses else torch.zeros((0,),device=device,dtype=torch.float32)
					scored_nll=scored_nll.to(torch.float64);loss_sum+=scored_nll.sum();token_count+=float(wlen-s);tb=val_data.base_bytes_lut[tgt].to(torch.float64);tb+=(val_data.has_leading_space_lut[tgt]&~val_data.is_boundary_token_lut[prev]).to(torch.float64);byte_count+=tb.sum()
