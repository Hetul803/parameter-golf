...
class Hyperparameters:
    ...
    # Eval-only recency lexical cache (sliding / sliding-TTT paths). Env mirrors typical CLI flags:
    #   USE_RECENCY_CACHE=1, RECENCY_CACHE_SIZE=4096, RECENCY_CACHE_STRENGTH=0.5, RECENCY_CACHE_DECAY=0.99
    #   RECENCY_CACHE_TOPK=0 (full-vocab bias), RECENCY_CACHE_MIN_COUNT=0 (top-k mode threshold)
    # USE_RECENCY_CACHE=0 (default): sliding eval is baseline — same logits and CE as before the cache existed.
    use_recency_cache = bool(int(os.environ.get("USE_RECENCY_CACHE", "0")))
    recency_cache_size = int(os.environ.get("RECENCY_CACHE_SIZE", 4096))
    recency_cache_strength = float(os.environ.get("RECENCY_CACHE_STRENGTH", 0.5))
    recency_cache_decay = float(os.environ.get("RECENCY_CACHE_DECAY", 0.99))
    recency_cache_topk = int(os.environ.get("RECENCY_CACHE_TOPK", "0"))
    recency_cache_min_count = float(os.environ.get("RECENCY_CACHE_MIN_COUNT", "0.0"))
...
# --- Recency lexical cache (eval-only; not used in training or export) ---

def recency_lexical_logit_bias(rows: Tensor, strength: float, topk: int, min_count: float) -> Tensor:
    """Build (L, V) additive logits from weighted continuation counts ``rows`` (eval-only).

    ``topk == 0``: full-vocab ``strength * log1p(rows)`` (same as the original cache).
    ``topk > 0``: only the top-``topk`` continuations per row with count >= ``min_count`` get that bias; rest stay 0.
    """
    s = float(strength)
    if topk <= 0:
        return (s * torch.log1p(rows)).to(dtype=torch.float32)
    _, v = rows.shape
    k_take = min(int(topk), v)
    masked = rows.masked_fill(rows < float(min_count), float("-inf"))
    vals, idx = torch.topk(masked, k=k_take, dim=-1)
    bias = torch.zeros(L, v, device=rows.device, dtype=torch.float32)
    finite = torch.isfinite(vals)
    contrib = torch.where(
        finite,
        s * torch.log1p(vals.clamp(min=0.0)),
        torch.zeros_like(vals),
    )
    # Selective top-k recency bias: only scatter onto the chosen continuation ids (safer than full-row push).
    bias.scatter_(-1, idx, contrib)
    return bias


class RecencyLexicalCache:
    """Rolling (prev_token -> next_token) history for optional eval-time logit bias.

    Stores up to ``max_pairs`` transitions in arrival order. Weight of an entry
    decays by ``decay`` per step toward older entries when rebuilding counts.
    """

    def __init__(self, vocab_size: int, device: torch.device, max_pairs: int, decay: float):
        self.vocab_size = vocab_size
        self.device = device
        self.max_pairs = max(1, max_pairs)
        self.decay = float(decay) if decay > 0 else 0.99
        self.buf: deque[tuple[int, int]] = deque(maxlen=self.max_pairs)

    def counts_matrix(self) -> Tensor:
        """Weighted co-occurrence matrix from the deque (most recent = largest weight)."""
        c = torch.zeros(self.vocab_size, self.vocab_size, device=self.device, dtype=torch.float32)
        for t, (p, n) in enumerate(reversed(self.buf)):
            c[p, n] += self.decay ** t
        return c

    def observe_pairs(self, prev_ids: Tensor, next_ids: Tensor) -> None:
        """Append (prev, next) pairs in tensor iteration order (CPU list walk)."""
        p_list = prev_ids.reshape(-1).detach().cpu().tolist()
        n_list = next_ids.reshape(-1).detach().cpu().tolist()
        for p, n in zip(p_list, n_list):
            self.buf.append((int(p), int(n)))

    def bias_for_prev_ids(
        self,
        prev_ids: Tensor,
        strength: float,
        ...
        """Return (L, vocab) bias for each previous-token row in the count matrix.

        If ``topk`` is 0, bias every vocab entry with ``strength * log1p(count)`` (original behavior).
        If ``topk`` > 0, bias only the up-to-``topk`` largest counts per row that are >= ``min_count``.
        """
        c = self.counts_matrix() if counts is None else counts
        rows = c[prev_ids.long().clamp(0, self.vocab_size - 1)]
        return recency_lexical_logit_bias(rows, strength, topk, min_count)


# First sliding eval with recency cache on logs hyperparameters once per process (rank 0 only).
_RECENCY_CACHE_EVAL_HP_LOGGED: bool = False
...
def eval_val_sliding(
    ...
    batch_seqs: int = 32,
    eval_seq_len: int | None = None,
    log0: Callable[[str], None] | None = None,
) -> tuple[float, float]:
    """Sliding window evaluation: each token scored with maximum context."""
    global _RECENCY_CACHE_EVAL_HP_LOGGED
    seq_len = eval_seq_len or args.train_seq_len
    ...
    recency: RecencyLexicalCache | None = None
    if args.use_recency_cache:
        recency = RecencyLexicalCache(
            args.vocab_size,
            device,
            max_pairs=args.recency_cache_size,
            decay=args.recency_cache_decay,
        )
        if rank == 0 and not _RECENCY_CACHE_EVAL_HP_LOGGED:
            hp_msg = (
                "recency_cache_eval: enabled "
                f"size={args.recency_cache_size} strength={args.recency_cache_strength} "
                f"decay={args.recency_cache_decay} topk={args.recency_cache_topk} "
                f"min_count={args.recency_cache_min_count}"
            )
            if log0 is not None:
                log0(hp_msg)
            else:
                print(hp_msg, flush=True)
            _RECENCY_CACHE_EVAL_HP_LOGGED = True
    with torch.inference_mode():
        for bi in range(0, len(my_windows), batch_seqs):
            ...
            if recency is None:
                ...
                for i, ws in enumerate(batch_ws):
                    wlen = wlens[i]
                    s = 0 if ws == 0 else max(wlen - stride, 0)
                    prev_slice = x_batch[i, s:wlen]
                    next_slice = y_batch[i, s:wlen]
                    logit_slice = logits[i, s:wlen, :].float()
                    counts = recency.counts_matrix()
                    bias = recency.bias_for_prev_ids(
                        prev_slice,
                        args.recency_cache_strength,
                        counts=counts,
                        topk=args.recency_cache_topk,
                        min_count=args.recency_cache_min_count,
                    ).to(device=device, dtype=logit_slice.dtype)
                    # Eval-time recency bias before CE: full vocab if RECENCY_CACHE_TOPK=0, else top-k continuations only.
                    adjusted = logit_slice + bias
                    ...
                    tb += (has_leading_space_lut[tgt] & ~is_boundary_token_lut[prev]).to(torch.float64)
                    byte_count += tb.sum()
                    # Cache update strictly after scoring these targets (bias above used pre-update counts only).
                    recency.observe_pairs(prev_slice, next_slice)
    if dist.is_available() and dist.is_initialized():
...
def eval_val_sliding_ttt(
    ...
    """Sliding eval hook for TTT-style call sites. This record has no separate TTT loop; delegates to eval_val_sliding.

    Recency cache (if enabled via args.use_recency_cache) uses the same sliding-window path.
    """
